...
#define GET_XBUTTON_WPARAM(wParam) (HIWORD(wParam))
	 ...
	 * button.
	 */
        if (repeated_click && ((button == MOUSE_LEFT && s_button_pending == MOUSE_RIGHT) || (button == MOUSE_RIGHT && s_button_pending == MOUSE_LEFT)))
        {
            /*
        ...
        else if ((repeated_click) || (mouse_model_popup() && (button == MOUSE_RIGHT)))
        {
    ...
    UINT keyFlags)
{
	 ...
	 * down.
	 */
        if (!(keyFlags & (MK_LBUTTON | MK_MBUTTON | MK_RBUTTON | MK_XBUTTON1 | MK_XBUTTON2)))
        {
            gui_mouse_moved(x, y);
            return;
        }
...
#ifdef FEAT_BEVAL_GUI
    case WM_NOTIFY:
        Handle_WM_Notify(hwnd, (LPNMHDR)lParam);
        return TRUE;
#endif
    default:
...
process_message(void)
{
	 ...
	 */
        ...
        for (i = 0; special_keys[i].key_sym != 0; i++)
        {
            ...
            if (special_keys[i].key_sym == vk && (vk != VK_SPACE || !(GetKeyState(VK_MENU) & 0x8000)))
            {
                /*
		 ...
		 */
                ...
                if (vk == VK_F10 && gui.menu_is_active && check_map(k10, State, FALSE, TRUE, FALSE, NULL, NULL) == NULL)
                    break;
#endif
                if (GetKeyState(VK_SHIFT) & 0x8000)
                    modifiers |= MOD_MASK_SHIFT;
		...
		*/
        ...
        if (special_keys[i].key_sym == 0)
        {
            ...
            if (vk != 0xff && (GetKeyState(VK_CONTROL) & 0x8000) && !(GetKeyState(VK_SHIFT) & 0x8000) && !(GetKeyState(VK_MENU) & 0x8000))
            {
...
void gui_mch_update(void)
{
    ...
    if (!s_busy_processing)
        while (pPeekMessage(&msg, NULL, 0, 0, PM_NOREMOVE) && !vim_is_input_buf_full())
            process_message();
}
...
int gui_mch_wait_for_chars(int wtime)
{
    ...
    while (!s_timed_out)
    {
        ...
        for (;;)
        {
            ...
            else if (input_available() || MsgWaitForMultipleObjects(0, NULL, FALSE, 100,
...
{
    return (guicolor_T)((GetRValue(pixel) << 16) + (GetGValue(pixel) << 8) + GetBValue(pixel));
}
    ...
    int *valid_h)
{
    ...
    base_width = gui_get_base_width() + (GetSystemMetrics(SM_CXFRAME) +
                                         GetSystemMetrics(SM_CXPADDEDBORDER)) *
                                            2;
    base_height = gui_get_base_height() + (GetSystemMetrics(SM_CYFRAME) + GetSystemMetrics(SM_CXPADDEDBORDER)) * 2 + GetSystemMetrics(SM_CYCAPTION)
#ifdef FEAT_MENU
...
logfont2name(LOGFONTW lf)
{
    ...
    res = alloc(strlen(font_name) + 30 + (charset_name == NULL ? 0 : strlen(charset_name) + 2) + (quality_name == NULL ? 0 : strlen(quality_name) + 2));
    if (res != NULL)
    {
        p = res;
...
update_im_font(void)
{
    ...
    if (p_guifontwide != NULL && *p_guifontwide != NUL && gui.wide_font != NOFONT && GetObjectW((HFONT)gui.wide_font, sizeof(lf_wide), &lf_wide))
...
void gui_mch_wide_font_changed(void)
{
    ...
    if (gui.wide_font && GetObjectW((HFONT)gui.wide_font, sizeof(lf), &lf))
    {
        if (!lf.lfItalic)
        {
            lf.lfItalic = TRUE;
 ...
 */
int gui_mch_init_font(char_u *font_name, int fontset UNUSED)
{
    LOGFONTW lf;
    GuiFont font = NOFONT;
    char_u *p;
...
void gui_mch_newfont(void)
{
    ...
    if (win_socket_id == 0)
    {
        gui_resize_shell(rect.right - rect.left - (GetSystemMetrics(SM_CXFRAME) + GetSystemMetrics(SM_CXPADDEDBORDER)) * 2,
                         rect.bottom - rect.top - (GetSystemMetrics(SM_CYFRAME) + GetSystemMetrics(SM_CXPADDEDBORDER)) * 2 - GetSystemMetrics(SM_CYCAPTION)
#ifdef FEAT_MENU
                             - gui_mswin_get_menu_height(FALSE)
#endif
        );
    ...
    const LPWINDOWPOS lpwpos)
{
    ...
    if (WSInitialized && (lpwpos->x != x || lpwpos->y != y || lpwpos->cx != cx || lpwpos->cy != cy))
    {
        x = lpwpos->x;
        y = lpwpos->y;
        cx = lpwpos->cx;
    ...
    LPRECT lprc)
{
    ...
    if (fwSide == WMSZ_TOP || fwSide == WMSZ_TOPLEFT || fwSide == WMSZ_TOPRIGHT)
        lprc->top += h_offset;
    else if (fwSide == WMSZ_BOTTOM || fwSide == WMSZ_BOTTOMLEFT || fwSide == WMSZ_BOTTOMRIGHT)
    ...
    LPARAM lParam)
{
    ...
    */
	 ...
	 */
#ifdef FEAT_MENU
        if (!gui.menu_is_active || p_wak[0] == 'n' || (p_wak[0] == 'm' && !gui_is_menu_shortcut((int)wParam)))
#endif
        {
        ...
        case TTN_GETDISPINFO:
        {
...
#ifdef FEAT_GUI_TABLINE
            if (gui_mch_showing_tabline() && hdr->hwndFrom == TabCtrl_GetToolTips(s_tabhwnd))
            {
                POINT pt;
                /*
            ...
            if (str != NULL)
            {
                ...
                else
                {
                    ...
                    if (STRLEN(str) < sizeof(lpdi->szText) || ((tt_text = vim_strsave(str)) == NULL))
...
#ifdef FEAT_GUI_TABLINE
        case TCN_SELCHANGE:
            if (gui_mch_showing_tabline() && ((LPNMHDR)lParam)->hwndFrom == s_tabhwnd)
            {
    ...
    case WM_MENUSELECT:
        if (((UINT)HIWORD(wParam) & (0xffff ^ (MF_MOUSESELECT + MF_BITMAP + MF_POPUP))) == MF_HILITE && (State & CMDLINE) == 0)
        {
            UINT idButton;
            vimmenu_T *pMenu;
            ...
            pMenu = gui_mswin_find_menu(root_menu, idButton);
            if (pMenu != NULL && pMenu->strings[MENU_INDEX_TIP] != 0 && GetMenuState(s_menuBar, pMenu->id, MF_BYCOMMAND) != -1)
            {
                ++msg_hist_off;
...
void gui_mch_prepare(int *argc, char **argv)
{
    ...
    if ((*argc == 2 || *argc == 3) && (argv[1][0] == '-' || argv[1][0] == '/'))
    {
        ...
        if (*argc == 3 && STRICMP(argv[1] + 1, "silent") == 0 && (argv[2][0] == '-' || argv[2][0] == '/'))
        {
            silent = TRUE;
            idx = 2;
        }
...
int gui_mch_init(void)
{
    ...
    if (vim_parent_hwnd != NULL)
    {
        ...
        __try
        {
            ...
            s_hwnd = CreateWindowExW(
                ...
                WS_OVERLAPPEDWINDOW | WS_CHILD | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | 0xC000,
    ...
    else
    {
        ...
        s_hwnd = CreateWindowW(
            szVimWndClassW, L"Vim MSWindows GUI",
            (win_socket_id == 0 ? WS_OVERLAPPEDWINDOW : WS_POPUP) | WS_CLIPSIBLINGS | WS_CLIPCHILDREN,
            gui_win_x == -1 ? CW_USEDEFAULT : gui_win_x,
            gui_win_y == -1 ? CW_USEDEFAULT : gui_win_y,
            100, // Any value will do
            100, // Any value will do
    ...
    int direction)
{
    ...
    win_width = width + (GetSystemMetrics(SM_CXFRAME) +
                         GetSystemMetrics(SM_CXPADDEDBORDER)) *
                            2;
    win_height = height + (GetSystemMetrics(SM_CYFRAME) + GetSystemMetrics(SM_CXPADDEDBORDER)) * 2 + GetSystemMetrics(SM_CYCAPTION)
#ifdef FEAT_MENU
                 + gui_mswin_get_menu_height(FALSE)
#endif
        ;
    ...
    if ((direction & RESIZE_HOR) && wndpl.rcNormalPosition.right > workarea_rect.right)
        OffsetRect(&wndpl.rcNormalPosition,
                   workarea_rect.right - wndpl.rcNormalPosition.right, 0);

    if ((direction & RESIZE_HOR) && wndpl.rcNormalPosition.left < workarea_rect.left)
        OffsetRect(&wndpl.rcNormalPosition,
                   workarea_rect.left - wndpl.rcNormalPosition.left, 0);

    if ((direction & RESIZE_VERT) && wndpl.rcNormalPosition.bottom > workarea_rect.bottom)
        OffsetRect(&wndpl.rcNormalPosition,
                   0, workarea_rect.bottom - wndpl.rcNormalPosition.bottom);

    if ((direction & RESIZE_VERT) && wndpl.rcNormalPosition.top < workarea_rect.top)
        OffsetRect(&wndpl.rcNormalPosition,
                ...
                else
                {
                                                ...
                                                &dwSentenceSaved))
                    {
                        ...
                        pImmSetConversionStatus(hImc,
                                                dwConversionSaved & ~(IME_CMODE_NATIVE | IME_CMODE_FULLSHAPE),
                                                dwSentenceSaved);
                    }
                }
            }
    ...
    int flags)
{
    ...
    if ((enc_utf8 || (enc_codepage > 0 && (int)GetACP() != enc_codepage) || enc_latin9) && (unicodebuf == NULL || len > unibuflen))
    {
        vim_free(unicodebuf);
...
void gui_mch_get_screen_dimensions(int *screen_w, int *screen_h)
{
    ...
    *screen_w = workarea_rect.right - workarea_rect.left - (GetSystemMetrics(SM_CXFRAME) + GetSystemMetrics(SM_CXPADDEDBORDER)) * 2;

    // FIXME: dirty trick: Because the gui_get_base_height() doesn't include
    // the menubar for MSwin, we subtract it from the screen height, so that
    // the window size can be made to fit on the screen.
    *screen_h = workarea_rect.bottom - workarea_rect.top - (GetSystemMetrics(SM_CYFRAME) + GetSystemMetrics(SM_CXPADDEDBORDER)) * 2 - GetSystemMetrics(SM_CYCAPTION)
#ifdef FEAT_MENU
                - gui_mswin_get_menu_height(FALSE)
#endif
        ;
...
#endif
    {
        if (menu->parent != NULL && menu_is_popup(menu->parent->dname) && menu->parent->submenu_id != NULL)
        ...
        if (menu->parent != NULL && menu->parent->children != NULL && IsWindow(menu->parent->tearoff_handle))
        {
...
rebuild_tearoff(vimmenu_T *menu)
{
    ...
    if (GetWindowRect(thwnd, &trect) && GetWindowRect(s_hwnd, &rct) && GetClientRect(s_hwnd, &roct))
    {
    ...
    int ex_cmd UNUSED)
{
    ...
    else
    {
        ...
        GetWindowRect(s_hwnd, &rect);
        maxDialogWidth = rect.right - rect.left - (GetSystemMetrics(SM_CXFRAME) + GetSystemMetrics(SM_CXPADDEDBORDER)) * 2;
        if (maxDialogWidth < DLG_MIN_MAX_WIDTH)
            maxDialogWidth = DLG_MIN_MAX_WIDTH;

        maxDialogHeight = rect.bottom - rect.top - (GetSystemMetrics(SM_CYFRAME) + GetSystemMetrics(SM_CXPADDEDBORDER)) * 4 - GetSystemMetrics(SM_CYCAPTION);
    ...
    do
    {
        ...
        {
            l = (*mb_ptr2len)(pend);
            if (l == 1 && VIM_ISWHITE(*pend) && textWidth > maxDialogWidth * 3 / 4)
                last_white = pend;
    ...
    dlgwidth = messageWidth + DLG_ICON_WIDTH + 3 * dlgPaddingX + GetSystemMetrics(SM_CXVSCROLL);
    ...
    // Dialog height.
    if (vertical)
        dlgheight = msgheight + 2 * dlgPaddingY + DLG_VERT_PADDING_Y + 2 * fontHeight * numButtons;
    else
    ...
    nchar = nCopyAnsiToWideChar(p, (title ? (LPSTR)title : (LPSTR)("Vim " VIM_VERSION_MEDIUM)), TRUE);
    p += nchar;
...
get_toolbar_bitmap(vimmenu_T *menu)
{
	 ...
	 */
        if (hbitmap == NULL && (gui_find_bitmap(
...
dyn_imm_load(void)
{
    ...
    pImmGetCompositionStringA = (void *)GetProcAddress(hLibImm, "ImmGetCompositionStringA");
    pImmGetCompositionStringW = (void *)GetProcAddress(hLibImm, "ImmGetCompositionStringW");
    pImmGetContext = (void *)GetProcAddress(hLibImm, "ImmGetContext");
    pImmAssociateContext = (void *)GetProcAddress(hLibImm, "ImmAssociateContext");
    pImmReleaseContext = (void *)GetProcAddress(hLibImm, "ImmReleaseContext");
    pImmGetOpenStatus = (void *)GetProcAddress(hLibImm, "ImmGetOpenStatus");
    pImmSetOpenStatus = (void *)GetProcAddress(hLibImm, "ImmSetOpenStatus");
    pImmGetCompositionFontW = (void *)GetProcAddress(hLibImm, "ImmGetCompositionFontW");
    pImmSetCompositionFontW = (void *)GetProcAddress(hLibImm, "ImmSetCompositionFontW");
    pImmSetCompositionWindow = (void *)GetProcAddress(hLibImm, "ImmSetCompositionWindow");
    pImmGetConversionStatus = (void *)GetProcAddress(hLibImm, "ImmGetConversionStatus");
    pImmSetConversionStatus = (void *)GetProcAddress(hLibImm, "ImmSetConversionStatus");

    if (pImmGetCompositionStringA == NULL || pImmGetCompositionStringW == NULL || pImmGetContext == NULL || pImmAssociateContext == NULL || pImmReleaseContext == NULL || pImmGetOpenStatus == NULL || pImmSetOpenStatus == NULL || pImmGetCompositionFontW == NULL || pImmSetCompositionFontW == NULL || pImmSetCompositionWindow == NULL || pImmGetConversionStatus == NULL || pImmSetConversionStatus == NULL)
    {
        FreeLibrary(hLibImm);
        hLibImm = NULL;
        pImmGetContext = NULL;
...
void gui_mch_disable_beval_area(BalloonEval *beval UNUSED)
{
